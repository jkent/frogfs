...
try {
	...
	let result = null;
	if (platform == 'win32') {
		result = spawnSync('cmd', ['/C', 'npm', '--prefix=' + env.NODE_PREFIX, 'install', '@babel/core', 'babel-preset-minify'], {'stdio': ['ignore', 'ignore', 'inherit']});
	} else {
		result = spawnSync('npm', ['--prefix=' + env.NODE_PREFIX, 'install', '@babel/core', 'babel-preset-minify'], {'stdio': ['ignore', 'ignore', 'inherit']});
	}
	if (result.status != 0) {
