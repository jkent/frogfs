...
macro(generate_frogfs_rules path)
    ...
    add_custom_command(OUTPUT ${Python3_VENV}_requirements.stamp
        COMMAND ${Python3_VENV}/bin/python -m pip install -r ${frogfs_DIR}/requirements.txt
    ...
    add_custom_target(frogfs_preprocess_${ARG_NAME}
        ...
        COMMAND ${CMAKE_COMMAND} -E env BUILD_DIR=${BUILD_DIR} ${Python3_VENV}/bin/python ${frogfs_DIR}/tools/preprocess.py ${ARG_CONFIG} ${path} ${output}
    ...
    add_custom_command(OUTPUT ${output}.bin
        COMMAND ${CMAKE_COMMAND} -E env BUILD_DIR=${BUILD_DIR} ${Python3_VENV}/bin/python ${frogfs_DIR}/tools/mkfrogfs.py ${SKIP_DIRS} ${output} ${output}.bin
...
function(target_add_frogfs target path)
    ...
    add_custom_command(OUTPUT ${output}_bin.c
        COMMAND ${Python3_VENV}/bin/python ${frogfs_DIR}/tools/bin2c.py ${output}.bin ${output}_bin.c
        DEPENDS ${output}.bin
